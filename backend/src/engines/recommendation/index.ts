import { Recommendation, PreprocessingOutput, ChunkingOutput, SchemaAnalysis, Snippet } from '../../types.ts';
 ...
 */
export const runRecommendationGeneration = async (input: RecommendationInput, traceId: string): Promise<Recommendation[]> => {
    logger.info(`Starting recommendation engine...`, traceId);
  ...
  "mainEntity": [{
    ...
    // Rule 2: Trust signals - missing meta description
    ...
    // Rule 3: Long chunks
    const longChunk = chunking.chunks.find(c => c.tokenCount > 700);
    if (longChunk) {
        recommendations.push({
            type: "content_fix",
            priority: "medium",
            target: `Section: '${longChunk.headingContext.slice(1).join(' > ')}'`,
            message: `Split this long section into smaller, focused chunks. AI engines prefer to cite paragraphs under 150 words.`,
    ...
    logger.info(`Recommendation engine complete. Generated ${recommendations.length} recommendations.`, traceId);
